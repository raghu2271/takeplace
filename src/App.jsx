...
const C = {
  ...
  muted:    "#64748B",
  soft:     "#94A3B8",
  // Light mode surfaces (for landing)
  lBg:      "#F8FAFC",
  lCard:    "#FFFFFF",
  lBorder:  "#E2E8F0",
  lText:    "#0F172A",
  ...
  return(
    <div className="room-fixed">
      ...
      <div style={{flex:1,background:"linear-gradient(180deg,#04060E 0%,#080C14 100%)",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
        ...
        {/* Live confidence metrics (right sidebar) */}
        {phase==="answering"&&(
        ...
        <div style={{position:"absolute",top:80,right:20,width:152,height:108,borderRadius:12,overflow:"hidden",border:"1px solid rgba(255,255,255,.12)",background:"#111",zIndex:15,boxShadow:"0 8px 32px rgba(0,0,0,.6)"}}>
          ...
          :<div style={{width:"100%",height:"100%",background:"#0d1220",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:5}}>
            ...
            <span style={{color:"rgba(255,255,255,.3)",fontSize:9}}>{camErr?"No camera":"Loading…"}</span>
          </div>}
...
function Dashboard({user,onStartInterview,onGoToJobs,stats}){
  ...
  // Build skill averages from history
  const skillAvg={
...
Return ONLY:
{"profile":{"name":"<from resume or Candidate>","topSkills":["s1","s2","s3","s4"],"keyProjects":["p1","p2","p3"],"experienceLevel":"<Fresher|Junior|Mid>","university":"<if found>"},"questions":[{"q":"<personalized spoken question>","type":"Intro|Technical|Behavioral|Closing","whyAsked":"<1 sentence>","keywords":["k1","k2","k3"]}]}`,
        2200
      );
  ...
  const startRec=()=>{
    ...
    rec.onresult=(e)=>{
      ...
      finalRef.current=finalAccum;setLiveText(finalAccum.trim());setInterimText(interimAccum.trim());
    };
    rec.onerror=()=>{};
  ...
  const beginQ=async(idx)=>{
    setPhase("speaking");setLiveText("");setInterimText("");setFeedback(null);setFillerCount(0);
    finalRef.current="";
...
{"overallScore":<0-100>,"verdict":"<Strong Hire|Hire|Borderline|No Hire>","communicationScore":<0-100>,"technicalScore":<0-100>,"confidenceScore":<0-100>,"structureScore":<0-100>,"resumeAlignmentScore":<0-100>,"summary":"<4-5 sentence brutally honest assessment>","companyFitNote":"<2 sentence about ${company||"company"} fit>","perQuestion":[{"question":"<short>","score":<0-100>,"feedback":"<2 sentence specific>","idealAnswer":"<2-3 sentence ideal STAR answer>"}],"strengths":["<s1>","<s2>","<s3>"],"improvements":["<i1>","<i2>","<i3>","<i4>"],"nextSteps":["<step1>","<step2>","<step3>"]}`,
      ...
      setReport(data);
      // Save to Supabase + update streak
...
Return ONLY: {"questions":[{"q":"<question>","type":"Intro|Technical|Behavioral|Closing","keywords":["k1","k2"]}]}`,900);
      const data=safeJSON(raw,null);
  ...
  const startRec=()=>{
    ...
    rec.onresult=(e)=>{
      let fa="",ia="";
      for(let i=0;i<e.results.length;i++){
        if(e.results[i].isFinal)fa+=e.results[i][0].transcript;
        else ia+=e.results[i][0].transcript;
      }
      finalRef.current=fa;setLiveText(fa.trim());setInterimText(ia.trim());
    };
    rec.onerror=()=>{};
    rec.onend=()=>setListening(false);
    recogRef.current=rec;
...
// ── JOBS TAB ──────────────────────────────────────────────────────────────────
function JobsTab({onPracticeForJob,resumeText}){
    ...
    <div style={{background:C.lBg,color:C.lText,fontFamily:"'Inter',sans-serif",overflowX:"hidden"}}>
      <style>{CSS}
        {`
          ...
          @keyframes borderGradient{0%,100%{border-color:rgba(124,110,250,.4)}50%{border-color:rgba(0,212,170,.4)}}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:60,maxWidth:1100,width:"100%",alignItems:"center",position:"relative",zIndex:1}}>
          ...
          <div>
            ...
            <h1 className="fade" style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:900,fontSize:"clamp(32px,4vw,56px)",lineHeight:1.08,marginBottom:24,animationDelay:".1s",letterSpacing:"-1px",color:"#fff"}}>
              You already know<br/>
              <span style={{background:"linear-gradient(135deg,#A89BFC,#00D4AA)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text"}}>what they'll ask.</span>
            </h1>
            ...
            <p className="fade" style={{fontSize:17,color:"rgba(255,255,255,.6)",lineHeight:1.8,maxWidth:480,marginBottom:40,animationDelay:".2s"}}>
              Because you just practiced it. Upload your resume, name the company — Priya asks the exact questions a real interviewer would ask <em>you specifically</em>. Camera on. Mic live. 90 seconds to answer. Full debrief at the end.
            </p>
            ...
            <div style={{background:"#04060E",borderRadius:20,border:"1px solid rgba(255,255,255,.1)",overflow:"hidden",boxShadow:"0 24px 60px rgba(0,0,0,.7)"}}>
              ...
              <div style={{padding:"24px",textAlign:"center",background:"linear-gradient(180deg,#07091A,#04060E)"}}>
                ...
                <div style={{background:"rgba(255,255,255,.04)",borderRadius:10,padding:"12px 14px",textAlign:"left",marginBottom:12}}>
                  ...
                  <div style={{color:"rgba(255,255,255,.85)",fontSize:11,lineHeight:1.65}}>Walk me through your TakePlace architecture — how did you handle CORS with the Groq API?</div>
      ...
      <div className="fade" style={{width:"100%",maxWidth:500,background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:24,padding:"36px 32px"}}>
        ...
        {step===0&&(
          <div>
            ...
            <div style={{color:C.soft,fontSize:13.5,marginBottom:24}}>We'll customize the interview style and difficulty to match.</div>
  ...
  useEffect(()=>{
    ...
    supabase.auth.getSession().then(({data:{session}})=>{
      if(session?.user){
        ...
        // Check if new user (no streak record)
        fetchUserStats(session.user.id).then(stats=>{
          if(stats&&stats.results.length===0)setShowOnboarding(true);
        });
        setPage("app");
      }
      setLoading(false);
