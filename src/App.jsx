...
function parseResumeStructure(rawText) {
    ...
    if (/B\.Tech|B\.E|M\.Tech|MBA|BCA|MCA|Bachelor|Master|B\.Sc|Intermediate|10th|12th|SSC|HSC/i.test(line)) {
      ...
      if (eduLines[i+1] && /University|College|Institute|School/i.test(eduLines[i+1])) {
        ...
        if (pipeSplit.length >= 2) {
          ...
          if (dateInRest) { dates = rest.slice(dateInRest.index).trim(); school = rest.slice(0, dateInRest.index).trim(); }
          else { school = rest.trim(); }
        }
        i++;
      }
  ...
  for (const line of expLines) {
    ...
    if (titleLine) {
      ...
      currentExp = { title:titleLine[1].trim(), company:titleLine[2].trim(), location:titleLine[3].trim(), dates:titleLine[4].trim(), bullets:[] };
  ...
  for (const line of skillLines) {
    ...
    if (colonIdx > 0) skills.push({ category:line.slice(0,colonIdx).trim(), items:line.slice(colonIdx+1).trim() });
  }
  const certLines = sections["CERTIFICATIONS"] || [];
  const certifications = certLines.map(l=>l.replace(/^[•\-\*]\s*/,"").trim()).filter(l=>l.length>5);
...
async function downloadPDF(resumeData, filename) {
  ...
  doc.text(contactParts.join("  |  "), W/2, y, {align:"center"}); y+=7;
  ...
  if(d.education?.length){
    ...
    d.education.forEach(edu=>{
      ...
      doc.text(edu.location||"", W-mr, y, {align:"right"}); y+=4.3;
      doc.setFont("helvetica","italic");
      doc.text(edu.degree||"", ml, y);
      doc.setFont("helvetica","normal");
      doc.text(edu.dates||"", W-mr, y, {align:"right"}); y+=5;
      doc.setFont("helvetica","normal");
    });
  }
  if(d.experience?.length){
    ...
    d.experience.forEach(exp=>{
      ...
      doc.setFont("helvetica","normal");
      doc.text(exp.dates||"", W-mr, y, {align:"right"}); y+=4.3;
      doc.setFont("helvetica","italic");
      doc.text(`${exp.company||""}${exp.location?", "+exp.location:""}`, ml, y); y+=4.3;
      doc.setFont("helvetica","normal");
      (exp.bullets||[]).forEach(b=>bullet(b)); y+=1;
    });
  }
  if(d.projects?.length){
    ...
    d.projects.forEach(proj=>{
      ...
      y+=4.3; doc.setFont("helvetica","normal");
      (proj.bullets||[]).forEach(b=>bullet(b)); y+=1;
    });
  }
  if(d.skills?.length){
    ...
    d.skills.forEach(sk=>{
      ...
      doc.text(skillText[0]||"", ml+catW, y); y+=4.5;
    });
  }
...
async function downloadDOCXJake(resumeData, filename) {
  ...
  if(d.education?.length){
    ...
    d.education.forEach(edu=>{
      children.push(new Paragraph({ children:[new TextRun({text:edu.school||"",bold:true,size:19,font:"Calibri"}),new TextRun({text:`\t${edu.location||""}`,size:19,font:"Calibri"})] }));
      children.push(new Paragraph({ children:[new TextRun({text:edu.degree||"",italics:true,size:18,font:"Calibri"}),new TextRun({text:`\t${edu.dates||""}`,size:18,font:"Calibri"})], spacing:{after:80} }));
    });
  }
  if(d.experience?.length){
    children.push(sectionPara("Experience"));
    d.experience.forEach(exp=>{
      children.push(new Paragraph({ children:[new TextRun({text:exp.title||"",bold:true,size:19,font:"Calibri"}),new TextRun({text:`\t${exp.dates||""}`,size:19,font:"Calibri"})] }));
      children.push(new Paragraph({ children:[new TextRun({text:`${exp.company||""}${exp.location?", "+exp.location:""}`,italics:true,size:18,font:"Calibri"})] }));
  ...
  if(d.projects?.length){
    ...
    d.projects.forEach(proj=>{
      children.push(new Paragraph({ children:[new TextRun({text:proj.name||"",bold:true,size:19,font:"Calibri"}), proj.tech?new TextRun({text:` | ${proj.tech}`,italics:true,size:19,font:"Calibri"}):null, proj.dates?new TextRun({text:`\t${proj.dates}`,size:19,font:"Calibri"}):null].filter(Boolean) }));
  ...
  if(d.skills?.length){
    ...
    d.skills.forEach(sk=>{
      children.push(new Paragraph({ children:[new TextRun({text:`${sk.category}: `,bold:true,size:18,font:"Calibri"}),new TextRun({text:sk.items||"",size:18,font:"Calibri"})] }));
    });
  }
  if(d.certifications?.length){
...
function LandingPage({ onGetStarted }) {
  ...
  const stats = [{ n:"50K+", label:"Jobs Listed" },{ n:"12K+", label:"Resumes Optimized" },{ n:"94%", label:"ATS Pass Rate" },{ n:"3x", label:"More Interviews" }];
  const features = [
    ...
    <div style={{ background:C.bg, color:C.text, fontFamily:"'Inter',sans-serif", overflowX:"hidden" }}>
      ...
      <section style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:"120px 24px 80px", position:"relative", overflow:"hidden", background:"linear-gradient(160deg,#eff6ff 0%,#ffffff 50%,#f0fdf4 100%)" }}>
        ...
        <div style={{ textAlign:"center", maxWidth:820, position:"relative", zIndex:1 }}>
          ...
          <div className="fade" style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap", animationDelay:".3s" }}>
            ...
            <Btn variant="ghost" onClick={()=>document.getElementById("features")?.scrollIntoView({behavior:"smooth"})} style={{ padding:"15px 28px", fontSize:16, borderRadius:12 }}>See How It Works ↓</Btn>
          </div>
      ...
      <section style={{ padding:"80px 24px", maxWidth:1100, margin:"0 auto" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:24 }}>
          {[
            ...
            { step:"02", icon:"🧠", title:"AI Analyzes Everything", desc:"AI scans keywords, scores your match, finds gaps, and judges every project for relevance." },
            ...
            <div key={i} className="hover-lift" style={{ background:"#ffffff", border:`1.5px solid ${C.border}`, borderRadius:20, padding:32, position:"relative", overflow:"hidden", boxShadow:"0 2px 12px rgba(0,0,0,0.05)" }}>
      ...
      <section id="features" style={{ padding:"80px 24px", background:"#f8fafc" }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:20 }}>
            {features.map((f,i)=>(
              <div key={i} className="hover-lift" style={{ background:"#ffffff", border:`1.5px solid ${C.border}`, borderRadius:18, padding:28, display:"flex", gap:18, boxShadow:"0 2px 8px rgba(0,0,0,0.04)" }}>
      ...
      <section style={{ padding:"60px 24px", background:"#eff6ff" }}>
        <div style={{ maxWidth:700, margin:"0 auto", textAlign:"center" }}>
          ...
          <div style={{ color:C.soft, fontSize:15, lineHeight:1.9, marginBottom:20 }}>
            TakePlace was built by <strong style={{color:C.blue}}>Raghu Dadigela</strong>, a B.Tech CSE (AI & ML) student who felt the real pain of job hunting — endless applications, ATS rejections, and resumes that never got shortlisted.
          </div>
          <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
            <Tag color={C.blue}>🎓 CSE AI & ML Graduate 2026</Tag>
            <Tag color={C.green}>⚡ Built with Claude AI + Supabase</Tag>
      ...
      <section style={{ padding:"60px 24px", maxWidth:700, margin:"0 auto", textAlign:"center" }}>
        <div style={{ background:"#ffffff", border:`1.5px solid ${C.border}`, borderRadius:20, padding:"40px 32px", boxShadow:"0 4px 20px rgba(0,0,0,0.06)" }}>
          ...
          <a href="mailto:takeplace.in@gmail.com" style={{ display:"inline-flex", alignItems:"center", gap:8, background:`linear-gradient(135deg,${C.blue},${C.blueLight})`, color:"#fff", fontWeight:700, padding:"12px 28px", borderRadius:10, fontSize:14, textDecoration:"none" }}>
            📧 takeplace.in@gmail.com
          </a>
        </div>
      </section>
      <section style={{ padding:"60px 24px", textAlign:"center", background:"#eff6ff" }}>
        <div style={{ maxWidth:580, margin:"0 auto", background:"#ffffff", border:`1.5px solid ${C.border}`, borderRadius:28, padding:"56px 40px", boxShadow:"0 8px 32px rgba(37,99,235,0.12)" }}>
      ...
      <footer style={{ borderTop:`1px solid ${C.border}`, padding:"24px", textAlign:"center", background:"#ffffff" }}>
        <div style={{ color:C.muted, fontSize:12 }}>© 2026 TakePlace · Developed by Raghu Dadigela · <a href="mailto:takeplace.in@gmail.com" style={{ color:C.blue, textDecoration:"none", fontWeight:600 }}>takeplace.in@gmail.com</a></div>
      </footer>
    </div>
  );
}
...
function AuthPage({ onLogin, onBack }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#eff6ff 0%,#ffffff 60%,#f0fdf4 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      <style>{css}</style>
      <div className="fade" style={{ width:"100%", maxWidth:420, background:"#ffffff", border:`1.5px solid ${C.border}`, borderRadius:24, padding:"36px 36px", boxShadow:"0 16px 48px rgba(37,99,235,0.12)" }}>
        ...
        <div style={{ display:"flex", background:C.card, borderRadius:10, padding:4, marginBottom:22 }}>
          {["login","register"].map(m=>(
            <button key={m} onClick={()=>{setMode(m);setErr("");setMsg("");}} style={{ flex:1, padding:"9px", borderRadius:8, border:"none", cursor:"pointer", fontFamily:"'Inter',sans-serif", fontWeight:600, fontSize:13, transition:"all .2s", background:mode===m?"#ffffff":"transparent", color:mode===m?C.blue:C.muted, boxShadow:mode===m?"0 1px 4px rgba(0,0,0,0.08)":"none" }}>
        ...
        <Btn variant="cta" onClick={handle} loading={loading} style={{ width:"100%", marginTop:20, padding:"13px", fontSize:15 }}>{mode==="login"?"Sign In →":"Create Account →"}</Btn>
      </div>
    </div>
  );
}
...
function ResumeAnalyzer({ user }) {
  ...
  // "original" | "suggested" | number index
...
Return ONLY valid JSON, no markdown fences:
{
  ...
  "strongSections": [
    {"section": "React.js Skills", "reason": "Directly listed in JD, proven with 2 projects"}
  ],
  "weakSections": [
    {"section": "No Docker/CI experience", "reason": "JD requires DevOps tooling, resume has none", "fix": "Add Docker basics to skills or a project"}
  ],
  "sectionAudit": [
  ...
  "projectFit": [
    {"name": "exact project name from resume", "relevance": 92, "keep": true, "reason": "Why relevant", "suggestion": "How to improve bullet points"}
  ],
...
"strongSections": 3-5 strongest areas comparing resume to JD.
"weakSections": 3-5 weakest gaps comparing resume to JD with actionable fixes.`;
      const raw = await callAI(prompt, 2200, "json");
...
- Underline strong JD keywords in bullets using <u>keyword</u> tags
...
[
  {
    ...
    "bullets": [
      "Built [specific thing] using <u>React.js</u>, serving 40+ beta users with 95% uptime",
      "Implemented <u>REST API</u> reducing manual effort by 22%",
      "Designed database schema with <u>MongoDB</u> handling 15+ endpoints, improving response time by 18%",
      "Deployed on Vercel with <u>CI/CD</u>, achieving zero-downtime over 8 iterations"
    ]
  }
]`;
      const raw = await callAI(prompt, 1500, "json");
      const arr = safeJSON(raw, []);
      if(Array.isArray(arr)&&arr.length>0) setSuggestedProjects(arr);
      else throw new Error("Could not generate suggestions.");
    } catch(e){ alert("Project suggestion failed: "+e.message); }
  ...
  const runOptimize = async (choiceOverride) => {
    ...
    // Determine projects to use
    let projectsForOptimize = parsed.projects;
    if(choice==="suggested"&&suggestedProjects?.length>0) projectsForOptimize = suggestedProjects;
    else if(typeof choice==="number"&&suggestedProjects?.[choice]) projectsForOptimize = [suggestedProjects[choice], ...parsed.projects.slice(0,2)];

    // Limit: 2 projects if has experience, 3 if no experience
    const maxProjects = hasExperience ? 2 : 3;
    projectsForOptimize = projectsForOptimize.slice(0, maxProjects);

    try {
      const expJSON = hasExperience ? JSON.stringify(parsed.experience.map(e=>({ title:e.title, company:e.company, location:e.location, dates:e.dates, originalBullets:e.bullets })), null, 2) : "[]";
      const projJSON = JSON.stringify(projectsForOptimize.map(p=>({ name:p.name, tech:p.tech||"", dates:p.dates, originalBullets:p.bullets||[] })), null, 2);

      const prompt = `You are an expert ATS resume bullet writer. Rewrite bullets using JD keywords. Underline strong keywords and bold metrics.
...
3. ONLY rewrite bullets — add JD keywords naturally
4. Keep same technical domain — don't change Python work to React/Node if original is Python
5. Realistic metrics only: 15-25% improvements; 40-100 users; not 500K
6. ${hasExperience ? "4 bullets per experience role, 4 bullets per project" : "4 bullets per project"}
...
8. Wrap JD-matching keywords in <u>keyword</u> tags for visual highlighting
9. Wrap ALL numeric metrics in <b>X%</b> or <b>N users</b> tags for bold emphasis
10. Example bullet: "Developed <u>REST API</u> using <u>Node.js</u> serving <b>40+ users</b>, reducing load time by <b>22%</b>"
  ...
  "experience": [
    {
      ...
      "bullets": ["bullet with <u>keywords</u> and <b>metrics</b>"]
    }
  ],
  "projects": [
    {
      ...
      // Education — use parsed exactly
      let education = parsed.education.length > 0
        ? parsed.education.map(e=>({ school:e.school||"", location:e.location||"", degree:e.degree||"", dates:e.dates||"" }))
        : [];
      if(education.length===0||education.every(e=>!e.school&&!e.degree)){
        try {
          const eduPrompt = `Extract ONLY education entries from this resume. Return JSON array only, no markdown, no other text.
Resume:
${resume.slice(0,1200)}
Format: [{"school":"Malla Reddy University, Hyderabad","location":"Hyderabad","degree":"B.Tech – Computer Science Engineering (AIML) | CGPA: 8.49 / 10","dates":"May 2026"}]
Include CGPA/percentage IN the degree string. Include all entries (degree, intermediate, etc).`;
          const eduRaw = await callAI(eduPrompt, 400, "json");
          const eduArr = safeJSON(eduRaw, []);
          if(Array.isArray(eduArr)&&eduArr.length>0&&eduArr[0].school) education = eduArr;
        } catch(_) {}
      }

      // Skills — use parsed, add JD skills
      ...
      if(skills.length===0){
        skills = [
          ...
          { category:"Concepts", items:"Data Structures, OOP, DBMS, SDLC" },
        ];
      }

      // Certifications — existing + role-based suggestions
      let existingCerts = parsed.certifications;
      if(existingCerts.length===0){
        const certSection = resume.match(/CERTIFICATIONS[\s\S]*?(?=\n[A-Z]{3,}|\z)/i);
        if(certSection) existingCerts = certSection[0].split("\n").slice(1).map(l=>l.replace(/^[•\-\*]\s*/,"").trim()).filter(l=>l.length>5);
      }

      // Generate role-based cert suggestions
      let roleCerts = [];
      try {
        const certPrompt = `Suggest 3-4 real, obtainable certification names that would strongly improve a resume for this job role.
JD summary: ${jdT.slice(0,300)}
Return ONLY a JSON array of strings, no markdown: ["Cert Name 1", "Cert Name 2", "Cert Name 3", "Cert Name 4"]
Make them real certifications from Google, AWS, Microsoft, Meta, Coursera, etc. that match this role exactly.`;
        const certRaw = await callAI(certPrompt, 300, "json");
        const certArr = safeJSON(certRaw, []);
        if(Array.isArray(certArr)) roleCerts = certArr.filter(c=>typeof c==="string"&&c.length>5).slice(0,4);
      } catch(_) {}

      // Merge: existing certs first, then role suggestions labelled
      const finalCerts = [
        ...existingCerts,
        ...roleCerts.map(c=>`${c} ⭐ Recommended for this role`),
      ];

      // Build optimized scores — target 90-95 based on JD match quality
      const baseMatch = analysis?.matchScore || 70;
      const baseAts = analysis?.atsScore || 70;
      // Higher the original score, closer to 95; lower original → bigger jump but cap at 92
      const optMatchScore = baseMatch >= 70 ? Math.min(95, baseMatch + 18) : Math.min(90, baseMatch + 22);
      const optAtsScore = baseAts >= 70 ? Math.min(95, baseAts + 16) : Math.min(92, baseAts + 20);
      const optShortlist = Math.min(52, (analysis?.shortlistRate||20) + 18);
      ...
      const optScores = { matchScore:optMatchScore, atsScore:optAtsScore, shortlistRate:optShortlist };
      setOptimized(finalResume);
      setOptimizedScores(optScores);
      setStep("optimized");
      setSection("resume");
      trackActivity("resume_optimized", `match:${optMatchScore}% hasExp:${hasExperience}`);
    ...
    return (
      <div style={{ textAlign:"center" }}>
        <svg width={size} height={size} viewBox="0 0 80 80">
          ...
          <circle cx="40" cy="40" r={r} fill="none" stroke={col} strokeWidth="6" strokeDasharray={circ} strokeDashoffset={circ*(1-score/100)} strokeLinecap="round" transform="rotate(-90 40 40)" style={{ transition:"stroke-dashoffset 1.2s ease" }}/>
  ...
  // Render bullet with underline (keywords) and bold (metrics) HTML tags as styled spans
  const BulletText = ({ text }) => {
    // Parse <u> and <b> tags
    const parts = [];
    let remaining = text;
    const tagRe = /(<u>.*?<\/u>|<b>.*?<\/b>)/g;
    let lastIndex = 0;
    let match;
    tagRe.lastIndex = 0;
    const matches = [...remaining.matchAll(/(<u>(.*?)<\/u>|<b>(.*?)<\/b>)/g)];
    if(matches.length===0) return <span>{text}</span>;
    let cursor = 0;
    matches.forEach((m, i) => {
      if(m.index > cursor) parts.push(<span key={`t${i}`}>{remaining.slice(cursor, m.index)}</span>);
      if(m[0].startsWith("<u>")) parts.push(<span key={`u${i}`} style={{ textDecoration:"underline", color:C.blue, fontWeight:600 }}>{m[2]}</span>);
      else if(m[0].startsWith("<b>")) parts.push(<span key={`b${i}`} style={{ fontWeight:800, color:C.green }}>{m[3]}</span>);
      cursor = m.index + m[0].length;
    });
    if(cursor < remaining.length) parts.push(<span key="end">{remaining.slice(cursor)}</span>);
    return <>{parts}</>;
  };

  // Jake's resume preview with underline/bold rendering
  const JakesResumePreview = ({ data }) => {
    if(!data) return null;
    const ps = { fontSize:8.5, lineHeight:"1.65", color:"#1a1a1a", marginBottom:2 };
    const sectionStyle = { borderBottom:"1.5px solid #1a1a1a", paddingBottom:1, marginBottom:6, marginTop:10, fontWeight:700, fontSize:9.5, letterSpacing:"0.06em", color:"#1a1a1a", textTransform:"uppercase" };
          ...
          <>
            ...
            {data.experience.map((exp,i)=>(
              <div key={i} style={{ marginBottom:8 }}>
                ...
                <div style={{ fontSize:8.5, fontStyle:"italic", color:"#374151", marginBottom:3 }}>{exp.company}{exp.location?`, ${exp.location}`:""}</div>
                {(exp.bullets||[]).map((b,j)=>(
                  <div key={j} style={bulletStyle}><span style={{ position:"absolute", left:3, top:0, fontSize:8.5 }}>•</span><BulletText text={b}/></div>
                ))}
              </div>
            ))}
          </>
        ...
        {data.certifications?.length>0&&(
          <>
            ...
            {data.certifications.map((c,i)=>{
              const isRec = c.includes("⭐ Recommended");
              const displayC = c.replace(" ⭐ Recommended for this role","");
              return (
                <div key={i} style={bulletStyle}>
                  <span style={{ position:"absolute", left:3, top:0, fontSize:8.5 }}>•</span>
                  <span style={isRec?{ color:"#7c3aed", fontWeight:600, fontSize:8.5 }:{}}>{displayC}</span>
                  {isRec&&<span style={{ background:"#ede9fe", color:"#7c3aed", fontSize:7, fontWeight:700, padding:"1px 5px", borderRadius:8, marginLeft:4 }}>Recommended</span>}
                </div>
              );
            })}
          </>
  ...
  if(step==="input") return (
    <div>
      ...
      <div style={{ background:"#f8f9fc", border:`1.5px solid ${C.border}`, borderRadius:16, padding:22, marginBottom:14 }}>
        ...
        <textarea value={jd} onChange={e=>{ setJd(e.target.value); localStorage.setItem("tp_jd",e.target.value); }} placeholder="Paste the full job description here..." style={{...inp, minHeight:180, resize:"vertical", lineHeight:1.8}}/>
      </div>
      <div style={{ background:"#f8f9fc", border:`1.5px solid ${C.border}`, borderRadius:16, padding:22, marginBottom:20 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
          ...
          <button onClick={()=>fileRef.current.click()} style={{ padding:"7px 14px", borderRadius:10, border:`1.5px solid ${C.blue}40`, background:`${C.blue}08`, color:C.blue, fontSize:12, cursor:"pointer", fontFamily:"'Inter',sans-serif", fontWeight:600 }}>
            📁 Upload PDF/DOCX
          </button>
        </div>
  ...
  if(step==="analyzing") return (
    <div style={{ textAlign:"center", padding:"80px 20px" }}>
      ...
      <div style={{ color:"#64748b", fontSize:14, lineHeight:1.9, marginBottom:28 }}>
        ...
        Identifying strong vs weak sections vs JD...
      </div>
  ...
  if(step==="optimizing") return (
    <div style={{ textAlign:"center", padding:"80px 20px" }}>
      ...
      <div style={{ color:"#64748b", fontSize:14, lineHeight:1.9, marginBottom:28 }}>
        ...
        Rewriting bullets with JD keywords · Adding <u>keyword highlights</u> · <b>Bold metrics</b>...<br/>
        Suggesting role-matched certifications · Jake format...
      </div>
  ...
  const tabs = [["overview","📊 Overview"], ...(step==="optimized"?[["resume","✨ Resume"]]:[] )];
  ...
  return (
    <div>
      ...
      <div style={{ background:"linear-gradient(135deg,#eff6ff,#f0fdf4)", border:`1.5px solid ${C.blue}20`, borderRadius:20, padding:24, marginBottom:16 }}>
        ...
        <div style={{ textAlign:"center" }}>
          <div style={{ display:"inline-block", padding:"6px 20px", borderRadius:20, background:scoreBg(displayScores.matchScore), color:scoreColor(displayScores.matchScore), fontWeight:800, fontSize:14, border:`1px solid ${scoreBorder(displayScores.matchScore)}`, marginBottom:10 }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16 }}>
        {tabs.map(([k,l])=>(
          <button key={k} onClick={()=>setSection(k)} style={{ padding:"9px 18px", borderRadius:20, whiteSpace:"nowrap", border:`1.5px solid ${section===k?C.blue:C.border}`, background:section===k?`${C.blue}10`:"#ffffff", color:section===k?C.blue:"#64748b", cursor:"pointer", fontFamily:"'Inter',sans-serif", fontWeight:section===k?700:400, fontSize:13, transition:"all .2s" }}>
            {l}
          </button>
        ))}
      </div>
      ...
      {section==="overview"&&(
        <div>
          ...
          {/* ── STRONG SECTIONS VS JD ── */}
          {(a.strongSections||[]).length>0&&(
            <div style={{ background:"#f0fdf4", border:`1.5px solid #bbf7d0`, borderRadius:16, padding:22, marginBottom:14 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
                <span style={{ fontSize:22 }}>💪</span>
                <div>
                  <div style={{ fontWeight:800, color:C.text, fontSize:16 }}>Strong Sections vs JD</div>
                  <div style={{ color:"#64748b", fontSize:12 }}>What's already working well in your resume for this role</div>
                </div>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))", gap:10 }}>
                {(a.strongSections||[]).map((s,i)=>(
                  <div key={i} style={{ background:"#ffffff", borderRadius:12, padding:14, border:"1px solid #bbf7d0", display:"flex", gap:12, alignItems:"flex-start" }}>
                    <span style={{ fontSize:18, flexShrink:0 }}>✅</span>
                    <div>
                      <div style={{ fontWeight:700, color:C.text, fontSize:13, marginBottom:3 }}>{s.section}</div>
                      <div style={{ color:"#475569", fontSize:12, lineHeight:1.6 }}>{s.reason}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* ── WEAK SECTIONS VS JD ── */}
          {(a.weakSections||[]).length>0&&(
            <div style={{ background:"#fef2f2", border:`1.5px solid #fecaca`, borderRadius:16, padding:22, marginBottom:14 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
                <span style={{ fontSize:22 }}>⚠️</span>
                <div>
                  <div style={{ fontWeight:800, color:C.text, fontSize:16 }}>Weak Sections vs JD</div>
                  <div style={{ color:"#64748b", fontSize:12 }}>Gaps between your resume and what this role needs</div>
                </div>
              </div>
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {(a.weakSections||[]).map((s,i)=>(
                  <div key={i} style={{ background:"#ffffff", borderRadius:12, padding:14, border:"1px solid #fecaca" }}>
                    <div style={{ fontWeight:700, color:"#dc2626", fontSize:13, marginBottom:4 }}>❌ {s.section}</div>
                    <div style={{ color:"#475569", fontSize:12, marginBottom:6 }}>{s.reason}</div>
                    {s.fix&&<div style={{ background:"#fffbeb", border:"1px solid #fef08a", borderRadius:8, padding:"7px 12px", fontSize:12, color:"#92400e" }}>💡 <strong>Fix:</strong> {s.fix}</div>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {(a.strongMatches||[]).length>0&&(
            <div style={{ background:"#f8f9fc", border:`1.5px solid ${C.border}`, borderRadius:16, padding:22, marginBottom:14 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
                ...
                <div style={{ fontWeight:700, color:C.text, fontSize:16 }}>Keyword Matches</div>
                <span style={{ background:"#f0fdf4", color:"#16a34a", fontSize:11, padding:"3px 10px", borderRadius:20, fontWeight:700 }}>{a.strongMatches.length} matched</span>
              </div>
          ...
          {(a.weakAreas||[]).length>0&&(
            <div style={{ background:"#f8f9fc", border:`1.5px solid ${C.border}`, borderRadius:16, padding:22, marginBottom:14 }}>
              <div style={{ fontWeight:700, color:C.text, fontSize:16, marginBottom:12 }}>⚡ Weak Areas to Fix</div>
              {a.weakAreas.map((w,i)=>(
          ...
          {/* ── PROJECT RELEVANCE ── */}
          <div style={{ background:"#f8f9fc", border:`1.5px solid ${C.border}`, borderRadius:16, padding:22, marginBottom:14 }}>
            <div style={{ fontWeight:700, color:C.text, fontSize:16, marginBottom:4 }}>🏗️ Project Relevance for This Role</div>
            <div style={{ color:"#64748b", fontSize:12, marginBottom:14 }}>Each project scored against the JD requirements</div>
            {(a.projectFit||[]).map((p,i)=>{
              const isStrong = p.relevance >= 65;
              return (
                <div key={i} style={{ background:isStrong?"#f0fdf4":"#fef2f2", borderRadius:12, padding:14, marginBottom:10, border:`1.5px solid ${isStrong?"#bbf7d0":"#fecaca"}` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                    ...
                    <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                      ...
                      <span style={{ background:isStrong?"#f0fdf4":"#fef2f2", color:isStrong?"#16a34a":"#dc2626", fontSize:11, padding:"3px 10px", borderRadius:20, fontWeight:700, border:`1px solid ${isStrong?"#bbf7d0":"#fecaca"}` }}>
                  ...
                  {p.suggestion&&<div style={{ background:`${C.purple}08`, border:`1px solid ${C.purple}20`, borderRadius:8, padding:"8px 12px", color:"#475569", fontSize:12 }}>💡 <strong style={{ color:C.purple }}>Tip:</strong> {p.suggestion}</div>}
                </div>
              );
            })}

            {/* PROJECT CHOICE — for weak projects */}
            {projectsAreWeak&&step!=="optimized"&&(
              <div style={{ background:"linear-gradient(135deg,#fdf4ff,#ede9fe)", border:`1.5px solid ${C.purple}30`, borderRadius:16, padding:20, marginTop:14 }}>
                <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
                  <span style={{ fontSize:22 }}>🚀</span>
                  <div>
                    <div style={{ fontWeight:800, color:C.text, fontSize:14 }}>Projects are weak for this role — choose an option:</div>
                    <div style={{ color:"#64748b", fontSize:12, marginTop:2 }}>AI will generate 3 stronger role-specific projects, or keep your originals</div>
                  </div>
                </div>
                <div style={{ display:"flex", gap:10, marginBottom:suggestedProjects?16:0, flexWrap:"wrap" }}>
                  <button onClick={()=>setProjectChoice("original")} style={{ padding:"10px 20px", borderRadius:10, border:`1.5px solid ${projectChoice==="original"?C.green:C.border}`, background:projectChoice==="original"?"#f0fdf4":"#fff", color:projectChoice==="original"?C.green:C.soft, fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"'Inter',sans-serif", transition:"all .2s" }}>
                    ✅ Keep My Original Projects
                  </button>
                  <button onClick={()=>{ setProjectChoice("suggested"); if(!suggestedProjects) suggestStrongerProjects(); }} disabled={suggestingProjects} style={{ padding:"10px 20px", borderRadius:10, border:`1.5px solid ${projectChoice==="suggested"?C.purple:C.border}`, background:projectChoice==="suggested"?`${C.purple}10`:"#fff", color:projectChoice==="suggested"?C.purple:C.soft, fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"'Inter',sans-serif", transition:"all .2s", display:"flex", alignItems:"center", gap:8 }}>
                    {suggestingProjects?<><SpinIcon size={12} color={C.purple}/> Generating...</>:"✨ Get AI-Suggested Stronger Projects"}
                  </button>
                </div>
                {suggestedProjects&&(
                  <div style={{ marginTop:14 }}>
                    <div style={{ fontWeight:700, color:C.text, fontSize:13, marginBottom:10 }}>Pick any project to use (or keep all 3), or keep originals:</div>
                    {suggestedProjects.map((proj,i)=>(
                      <div key={i} style={{ background:"#ffffff", border:`1.5px solid ${projectChoice===i?C.purple:C.border}`, borderRadius:12, padding:14, marginBottom:8, cursor:"pointer", transition:"all .2s" }} onClick={()=>setProjectChoice(i)}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
                          <div>
                            <div style={{ fontWeight:800, color:C.text, fontSize:13, display:"flex", alignItems:"center", gap:8 }}>
                              {projectChoice===i&&<span style={{ fontSize:14, color:C.purple }}>✓</span>}
                              {proj.name}
                            </div>
                        ...
                        {proj.whyStrong&&<div style={{ background:`${C.purple}08`, border:`1px solid ${C.purple}20`, borderRadius:8, padding:"6px 10px", marginBottom:8, fontSize:12, color:C.purple }}>💡 {proj.whyStrong}</div>}
                        {(proj.bullets||[]).map((b,j)=>(
            ...
            <div style={{ background:"linear-gradient(135deg,#eff6ff,#ede9fe)", border:`1.5px solid ${C.blue}20`, borderRadius:20, padding:24, textAlign:"center" }}>
              ...
              <div style={{ color:"#64748b", fontSize:13, marginBottom:16, lineHeight:1.7 }}>
                {projectsAreWeak&&projectChoice==="suggested"
                  ? "Using AI-suggested projects · Rewriting bullets with JD keywords · Underlined keywords · Bold metrics"
                  : "Keeping your original projects · Rewriting bullets with JD keywords · Underlined keywords · Bold metrics"}
                <br/>+ Role-matched certification suggestions added automatically
              </div>
              <button onClick={()=>runOptimize(projectChoice)} style={{ padding:"14px 40px", fontSize:15, borderRadius:12, border:"none", cursor:"pointer", background:"linear-gradient(135deg,#7c3aed,#5b21b6)", color:"#fff", fontWeight:800, fontFamily:"'Inter',sans-serif", boxShadow:`0 4px 16px ${C.purple}40` }}>
        ...
        <div>
          <div style={{ background:"#f8f9fc", border:`1.5px solid ${C.border}`, borderRadius:16, padding:22, marginBottom:16 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12, flexWrap:"wrap", gap:12 }}>
              <div>
                <div style={{ fontWeight:700, color:C.text, fontSize:16 }}>✨ Your Optimized Resume — Jake's ATS Format</div>
                <div style={{ color:"#64748b", fontSize:12, marginTop:2 }}>
                  <span style={{ color:C.blue, fontWeight:600 }}>Underlined</span> = JD keywords · <span style={{ color:C.green, fontWeight:700 }}>Bold green</span> = metrics
                </div>
              </div>
              <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                <button onClick={()=>handleDownload("pdf")} disabled={downloading==="pdf"} style={{ padding:"10px 18px", borderRadius:10, border:"none", cursor:"pointer", background:"linear-gradient(135deg,#5b21b6,#7c3aed)", color:"#fff", fontWeight:700, fontFamily:"'Inter',sans-serif", fontSize:13 }}>
                  {downloading==="pdf"?"⏳ Generating...":"⬇ Download PDF"}
                </button>
                <button onClick={()=>handleDownload("docx")} disabled={downloading==="docx"} style={{ padding:"10px 18px", borderRadius:10, border:"none", cursor:"pointer", background:"linear-gradient(135deg,#14532d,#16a34a)", color:"#fff", fontWeight:700, fontFamily:"'Inter',sans-serif", fontSize:13 }}>
              ...
              <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, background:"#eff6ff", border:`1px solid ${C.blue}20`, borderRadius:10, padding:"8px 14px" }}>
                  ...
                  <div>
                    ...
                    <div style={{ color:"#64748b", fontSize:11, display:"flex", alignItems:"center", gap:4 }}>was {a.matchScore}% <DeltaBadge original={a.matchScore} optimized={optimizedScores.matchScore}/></div>
                  </div>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:8, background:"#f0fdf4", border:"1px solid #16a34a20", borderRadius:10, padding:"8px 14px" }}>
                  ...
                  <div>
                    ...
                    <div style={{ color:"#64748b", fontSize:11, display:"flex", alignItems:"center", gap:4 }}>was {a.atsScore}% <DeltaBadge original={a.atsScore} optimized={optimizedScores.atsScore}/></div>
                  </div>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:8, background:"#fdf4ff", border:`1px solid ${C.purple}20`, borderRadius:10, padding:"8px 14px" }}>
                  <div style={{ width:50, height:50, borderRadius:"50%", border:`4px solid ${C.purple}`, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <span style={{ fontWeight:900, fontSize:13, color:C.purple }}>{optimizedScores.shortlistRate}%</span>
                  </div>
                  <div>
                    <div style={{ fontWeight:700, color:C.text, fontSize:13 }}>Shortlist Rate</div>
                    <div style={{ color:"#64748b", fontSize:11, display:"flex", alignItems:"center", gap:4 }}>was {a.shortlistRate||15}% <DeltaBadge original={a.shortlistRate||15} optimized={optimizedScores.shortlistRate}/></div>
                  </div>
                </div>
              </div>
            )}
          ...
          {/* Legend */}
          <div style={{ background:"#eff6ff", border:`1px solid ${C.blue}20`, borderRadius:10, padding:"10px 16px", marginBottom:14, fontSize:12, color:"#475569", display:"flex", gap:20, flexWrap:"wrap" }}>
            <span>🔵 <span style={{ textDecoration:"underline", color:C.blue, fontWeight:600 }}>Underlined blue</span> = JD keyword matched in bullet</span>
            <span>🟢 <strong style={{ color:C.green }}>Bold green</strong> = metric/number for ATS & recruiter impact</span>
            <span>🟣 <span style={{ color:C.purple, fontWeight:600 }}>Purple</span> = Role-recommended certification</span>
          </div>
      ...
      <div style={{ marginTop:18 }}>
        <button onClick={()=>{ setStep("input"); setAnalysis(null); setOptimized(null); setOptimizedScores(null); setErr(""); setSection("overview"); setJd(""); setResume(""); setFileName(""); setSuggestedProjects(null); setProjectChoice("original"); localStorage.removeItem("tp_jd"); localStorage.removeItem("tp_resume"); localStorage.removeItem("tp_fileName"); }} style={{ width:"100%", padding:"12px", borderRadius:10, border:`1.5px solid ${C.border}`, background:"transparent", color:"#64748b", cursor:"pointer", fontFamily:"'Inter',sans-serif", fontSize:13 }}>
          🔄 Analyze Another Job
        </button>
      </div>
    </div>
  ...
  async function handleDownload(type) {
    if(!optimized) return;
    setDownloading(type);
    // Strip HTML tags for PDF/DOCX download (plain text)
    const stripTags = (str) => (str||"").replace(/<[^>]+>/g,"");
    const cleanedResume = {
      ...optimized,
      experience: (optimized.experience||[]).map(e=>({ ...e, bullets:(e.bullets||[]).map(stripTags) })),
      projects: (optimized.projects||[]).map(p=>({ ...p, bullets:(p.bullets||[]).map(stripTags) })),
      certifications: (optimized.certifications||[]).map(c=>c.replace(" ⭐ Recommended for this role","").trim()),
    };
    try {
      if(type==="pdf") await downloadPDF(cleanedResume, "TakePlace_Optimized_Resume.pdf");
      else await downloadDOCXJake(cleanedResume, "TakePlace_Optimized_Resume.docx");
      ...
      <div style={{ background:"#ffffff", borderBottom:`1.5px solid ${C.border}`, position:"sticky", top:60, zIndex:99 }}>
        <div style={{ maxWidth:820, margin:"0 auto", display:"flex" }}>
          {TABS.map(([icon,label],i)=>(
            <button key={i} onClick={()=>setTabPersist(i)} style={{ flex:1, padding:"14px 6px", border:"none", background:"transparent", cursor:"pointer", color:tab===i?C.blue:C.muted, fontFamily:"'Inter',sans-serif", fontWeight:tab===i?800:500, fontSize:14, borderBottom:`2.5px solid ${tab===i?C.blue:"transparent"}`, transition:"all .2s", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
              {icon} {label}
            </button>
          ))}
        </div>
