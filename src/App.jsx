...
{"profile":{"name":"<from resume or Candidate>","topSkills":["s1","s2","s3","s4"],"keyProjects":["p1","p2","p3"],"experienceLevel":"<Fresher|Junior|Mid>","university":"<if found>"},"questions":[{"q":"<personalized spoken question>","type":"Intro|Technical|Behavioral|Closing","whyAsked":"<1 sentence>"}]}`,
      ...
      console.error("Resume question generation failed:",e);
      setGenErr("⚠ Couldn't reach the AI ("+(e.message||"unknown error")+"). Showing standard questions instead — check your /api/ai endpoint and Groq API key in Vercel.");
  ...
  const reactionLine=(score)=>{
    const strong=["Nice, that was a strong answer.","Good, I like that example.","Great, that's exactly what I was looking for."];
    const mid=["Okay, noted.","Alright, that works.","Got it, thanks."];
    const weak=["Hmm, let's work on that one.","Okay, that one was a bit thin.","Alright, noted — let's move on."];
    const pool=score>=75?strong:score>=50?mid:weak;
...
Return ONLY: {"score":<0-100>,"tip":"<2-3 sentence specific actionable feedback>","what_was_good":"<1 sentence about best part, or null if poor>","missing":"<what was missing from ideal answer>"}`,
      ...
      console.error("Feedback AI call failed:",e);
      fb={score:55,tip:"⚠ AI feedback service didn't respond ("+(e.message||"unknown error")+"). Check your /api/ai route / Groq key.",what_was_good:null};
    }
    setFeedback(fb);
...
{"overallScore":<0-100>,"verdict":"<Strong Hire|Hire|Borderline|No Hire>","communicationScore":<0-100>,"technicalScore":<0-100>,"confidenceScore":<0-100>,"structureScore":<0-100>,"resumeAlignmentScore":<0-100>,"summary":"<4-5 sentence brutally honest assessment>","companyFitNote":"<2 sentence about ${company||"company"} fit>","perQuestion":[{"question":"<short>","score":<0-100>,"feedback":"<2 sentence specific>"}],"strengths":["<s1>","<s2>","<s3>"],"improvements":["<i1>","<i2>","<i3>","<i4>"],"nextSteps":["<step1>","<step2>","<step3>"]}`,
      ...
      setReport({overallScore:0,verdict:"—",communicationScore:0,technicalScore:0,confidenceScore:0,structureScore:0,resumeAlignmentScore:0,summary:"⚠ Couldn't generate the report ("+(e.message||"unknown error")+"). Check your /api/ai endpoint and Groq API key, then retry.",companyFitNote:"",perQuestion:[],strengths:[],improvements:[],nextSteps:[]});
    }
    setStep("report");
  };
...
Return ONLY: {"questions":[{"q":"<question>","type":"Intro|Technical|Behavioral|Closing"}]}`,900);
      ...
      console.error("Quick mock question generation failed:",e);
      setGenErr("⚠ Couldn't reach the AI ("+(e.message||"unknown error")+"). Using standard questions — check /api/ai and your Groq key.");
      setQuestions(FALLBACK_QUESTIONS);
    }
    setLoadingQs(false);
  };
...
Return ONLY: {"overallScore":<0-100>,"verdict":"<Strong Hire|Hire|Borderline|No Hire>","communicationScore":<0-100>,"technicalScore":<0-100>,"confidenceScore":<0-100>,"structureScore":<0-100>,"summary":"<3-4 sentence honest assessment>","perQuestion":[{"question":"<short>","score":<0-100>,"feedback":"<2 sentence>"}],"strengths":["<s1>","<s2>"],"improvements":["<i1>","<i2>","<i3>"]}`,2200);
      ...
      console.error("Quick mock report generation failed:",e);
      setReport({overallScore:0,verdict:"—",communicationScore:0,technicalScore:0,confidenceScore:0,structureScore:0,summary:"⚠ Couldn't generate the report ("+(e.message||"unknown error")+"). Check /api/ai and your Groq key, then retry.",perQuestion:[],strengths:[],improvements:[]});
    }
      ...
      <section style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"130px 24px 90px",position:"relative",overflow:"hidden",background:`linear-gradient(160deg,${C.navy} 0%,${C.navyL} 45%,#1e3a6e 100%)`,color:"#fff"}}>
        ...
        <div style={{textAlign:"center",maxWidth:820,position:"relative",zIndex:1}}>
          ...
          <h1 className="fade" style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:900,fontSize:"clamp(36px,5.5vw,64px)",lineHeight:1.07,marginBottom:26,animationDelay:".1s",letterSpacing:"-1px"}}>
            Walk in already knowing<br/>
            what <span style={{color:C.goldL}}>they'll ask you</span>.
          </h1>
          ...
          <p className="fade" style={{fontSize:17,color:"rgba(255,255,255,.7)",lineHeight:1.85,maxWidth:580,margin:"0 auto 48px",animationDelay:".2s"}}>
            Paste your resume. Name the company — Google, Amazon, TCS, anyone. Our AI reads what you've actually built and runs a real camera interview around it: it speaks the questions, you answer live, and it scores you the moment you're done.
          </p>
